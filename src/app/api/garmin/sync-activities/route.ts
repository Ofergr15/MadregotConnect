...
import { findCrossSourceDuplicate, twinVerdict } from '@/lib/activity-dedup';
  ...
  try {
    ...
    for (const athlete of athletes) {
      ...
      try {
        ...
        const upgrades: Array<{ rowId: string; activity: (typeof candidateActivities)[number] }> = [];
        for (const a of candidateActivities) {
          ...
          else if (verdict === 'upgrade' && twin) upgrades.push({ rowId: twin.id, activity: a });
        ...
        if (newActivities.length + upgrades.length > 0) {
          ...
          // Replace each Strava twin's contents with the watch's own record of
          // the run. Same row, same id, so the feed item and its kudos survive;
          // `source` has to be written explicitly because the insert path leans
          // on the column default. `athlete_id` is dropped — it is already
          // correct on the row being replaced, and this is not the place to be
          // able to move a run between athletes.
          //
          ...
          for (let i = 0; i < upgrades.length; i++) {
            ...
            const { athlete_id: _owner, ...fields } = upgradeRows[i];
            let update: Record<string, any> = { ...fields, source: 'garmin' };
