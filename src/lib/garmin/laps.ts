...
/**
 * A heart rate, or nothing. Zero is not a reading — a lap the strap dropped comes
 * back 0, and the per-split HR chart drew that as a beat count of zero. Elevation
...
export function narrowLaps(lapDTOs: unknown): StoredLap[] {
  ...
  for (const lap of lapDTOs) {
    ...
    out.push({
      ...
      averageHR: bpm(l?.averageHR) ?? null,
      maxHR: bpm(l?.maxHR) ?? null,
...
export function normalizeStoredLaps(raw: unknown): StoredLap[] {
  ...
  for (const lap of raw) {
    ...
    out.push({
      ...
      duration,
      averagePace,
      averageHR: bpm(l.averageHR ?? l.average_heartrate) ?? null,
      maxHR: bpm(l.maxHR ?? l.max_heartrate) ?? null,
 ...
 *
 * The distinction matters because an empty array is a real, useful value here — it
 * is how a caller knows not to spend a Garmin request asking again. A column
 * nobody has filled is `null`, and that one is worth a request.
 */
