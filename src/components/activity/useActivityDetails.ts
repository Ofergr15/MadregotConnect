...
export function useActivityDetails({
  ...
  const [verdict, setVerdict] = useState<PlanVerdict | null>(null);
      ...
      if (planAthleteId && planStart) {
        ...
        try {
          ...
          if (pr.ok) {
            ...
            if (pj?.verdict) setVerdict(pj.verdict as PlanVerdict);
          }
  ...
  return { details, loading, error, planned, verdict, load };
}
