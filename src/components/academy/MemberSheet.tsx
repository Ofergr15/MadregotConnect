        ...
        <div dir="auto">
          ...
          <div className="flex items-center gap-3 pb-4">
            {member.avatarUrl ? (
              ...
              <div className="w-14 h-14 rounded-full bg-primary-600/20 flex items-center justify-center text-base font-bold text-primary-300 shrink-0">
                {initialsOf(member.name)}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <div className="text-base font-bold text-white truncate">{member.name}</div>
              <div className="text-xs text-slate-400 truncate" dir="ltr">{member.email}</div>
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                {member.groupName && (
                  <span className="text-2xs font-bold px-2 py-0.5 rounded-md bg-purple-500/15 text-purple-300 border border-purple-500/20">
                    {member.groupName}
                  </span>
                )}
                <span className={cn(
          ...
          <InsetSection header={t('memberFacts')}>
            <InsetRow icon={Activity} iconBg="bg-primary-600" label={t('weekRuns')} value={String(member.weekRuns)} />
            <InsetRow icon={Timer} iconBg="bg-sky-600" label={t('weekTime')} value={t('minutesValue', { min: member.weekDurationMin })} />
            <InsetRow icon={Route} iconBg="bg-emerald-600" label={t('allTimeKm')} value={member.totalKm.toFixed(1)} />
            <InsetRow icon={Activity} iconBg="bg-violet-600" label={t('allTimeRuns')} value={String(member.totalRuns)} />
            <InsetRow icon={Watch} iconBg="bg-amber-600" label={t('lastActivity')} value={lastSeen} />
          </InsetSection>

          {/* Per-workout planned-vs-actual for the selected week */}
          <p className="px-4 mb-1.5 text-2xs font-bold uppercase tracking-wider text-slate-500">
            {t('weekBreakdown')}
          </p>
          <div className="rounded-2xl bg-slate-800/80 border border-slate-700/50 overflow-hidden divide-y divide-slate-700/50 mb-5">
            {loadingWeek ? (
              <div className="flex items-center justify-center gap-2 py-6 text-xs text-slate-500">
                <Spinner size={16} /> {t('loading')}
              </div>
            ) : !workouts || workouts.length === 0 ? (
              <p className="px-4 py-5 text-center text-xs text-slate-500">{t('noPlannedWorkouts')}</p>
            ) : (
              workouts.map((w, i) => (
                <div key={i} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {w.completed ? (
                      ...
                      <div className="mt-0.5 text-xs text-slate-500">{t('notCompleted')}</div>
                    )}
                  </div>
                </div>
              ))
...
function StatTile({
  ...
  return (
    <div className="rounded-xl bg-slate-800/60 border border-slate-700/60 px-3 py-2.5 text-center">
      <Icon className="h-3.5 w-3.5 text-slate-500 mx-auto" />
      <div className={cn('mt-1 text-lg font-bold tabular-nums leading-none', valueClass || 'text-white')}>{value}</div>
      <div className="mt-1 text-2xs text-slate-500 leading-tight">{label}</div>
    </div>
  );
}
