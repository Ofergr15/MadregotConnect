    ...
    <div dir="rtl">
      ...
      <div className="flex items-center justify-center gap-3 mb-6">
        ...
        <div className="text-center min-w-[180px]">
          ...
          <div className="text-xs text-slate-500">{isCurrentWeek ? 'השבוע' : ''}</div>
        </div>
        <button
          onClick={() => setWeekStart(w => shiftWeek(w, 1))}
            ...
            return (
              <div key={a.athleteId} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center gap-4 p-4 text-start hover:bg-slate-800/70 transition-colors min-h-[44px]"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-white truncate" dir="auto">{a.name}</div>
                  ...
                  <div className={cn('text-center shrink-0 w-14', scoreColor(w.completionRate))}>
                    ...
                    <div className="text-[10px] text-slate-500 -mt-0.5">בוצע</div>
                  </div>
                </button>
                ...
                {open && (
                  <div className="border-t border-slate-700/50 divide-y divide-slate-700/30">
                    {w.workouts.length === 0 ? (
                      <div className="p-4 text-sm text-slate-500 text-center">אין אימונים מתוכננים השבוע.</div>
                    ) : (
                      w.workouts.map((wk, i) => {
                        ...
                        return (
                          <div key={i} className="p-4 flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="font-medium text-white text-sm truncate" dir="auto">{wk.name}</div>
                              {wk.completed ? (
                                <>
...
function SegmentsPanel({ athleteId, date }: { athleteId: string; date: string }) {
  ...
  return (
    <div className="mt-2">
      <button onClick={toggle} className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-400 hover:text-white min-h-[44px]">
        <ListChecks className="h-3.5 w-3.5" /> {open ? 'הסתרת מקטעים' : 'פירוט לפי מקטע'}
      </button>
      {open && (
        <div className="mt-2">
          {loading ? (
            <div className="flex items-center gap-2 text-xs text-slate-500 py-2"><Spinner size={14} /> טוען מקטעים…</div>
          ) : graded.length === 0 ? (
            <p className="text-[11px] text-slate-500 py-1">{reason || 'אין מקטעים מדורגים.'}</p>
          ) : (
            <div className="space-y-1">
              {segments!.map((s, i) => (
                <div key={i} className={cn('flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs',
                  ...
                  <span className="text-slate-400 flex-1 min-w-0 truncate" dir="auto">{s.label}</span>
                  {s.graded ? (
                    <>
                    ...
                    <span className="text-slate-600 w-16 text-end">—</span>
                  )}
                </div>
              ))}
            </div>
