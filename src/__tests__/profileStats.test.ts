...
// A run, given as the date it started on. Week boundaries are ACTIVITY weeks,
// which are Monday-based (the watch's week — re-split from the Sunday plan week
// on 2026-09-09): 2026-08-31, 2026-08-24 and 2026-08-17 are all Mondays.
...
describe('buildKmTable', () => {
  // The week keys these assertions expect are Monday-anchored local dates, and
  ...
  it('generates the window oldest-first ending on the current week', () => {
    const table = buildKmTable([], { limit: 4, currentWeekStart: '2026-08-31' });
    expect(table.map((r) => r.weekStart)).toEqual(['2026-08-10', '2026-08-17', '2026-08-24', '2026-08-31']);
  });
  ...
  it('always includes the current week, even with no runs in it at all', () => {
    ...
    const table = buildKmTable([run('2026-08-18', 12)], { limit: 3, currentWeekStart: '2026-08-31' });
    expect(table[table.length - 1]).toMatchObject({ weekStart: '2026-08-31', km: 0, runs: 0, isCurrent: true });
  });

  it('buckets runs into their Monday week and sums them', () => {
    const table = buildKmTable(
      [run('2026-08-31', 10), run('2026-09-02', 5.5), run('2026-08-27', 8)],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
    expect(table[0]).toMatchObject({ weekStart: '2026-08-24', km: 8, runs: 1 });
    expect(table[1]).toMatchObject({ weekStart: '2026-08-31', km: 15.5, runs: 2 });
  });
  ...
  it('ignores runs outside the window rather than folding them into an edge week', () => {
    ...
    const table = buildKmTable(
      [run('2026-01-06', 200), run('2026-08-31', 10)],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
    expect(table.map((r) => r.km)).toEqual([0, 10]);
  });

  it('keeps a late Sunday-evening run in the week it closes', () => {
    // Read as an instant, a 21:30 run shifts +3h in Israel and jumps into the next
    // week, moving kilometres between two adjacent columns. On a Mon–Sun activity
    // week Sunday is the closing day, so this is the run that would move:
    // 2026-08-30 is the Sunday that CLOSES the 2026-08-24 week.
    const table = buildKmTable(
      [{ ...run('2026-08-30', 9), start_time: '2026-08-30T21:30:00' }],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
    expect(table[0]).toMatchObject({ weekStart: '2026-08-24', km: 9 });
    expect(table[1].km).toBe(0);
  });

  it('computes the week-over-week delta against the previous column', () => {
    const table = buildKmTable(
      [run('2026-08-25', 20), run('2026-09-01', 23)],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
  ...
  it('gives the oldest column no delta', () => {
    ...
    const table = buildKmTable(
      [run('2026-08-18', 10), run('2026-08-25', 20)],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
    expect(table[0].weekStart).toBe('2026-08-24');
  ...
  it('reports no delta when the previous week was a zero week', () => {
    ...
    const table = buildKmTable([run('2026-09-01', 14)], { limit: 2, currentWeekStart: '2026-08-31' });
    expect(table[1].deltaPct).toBeNull();
  });

  it('reports a negative delta for a down week', () => {
    const table = buildKmTable(
      [run('2026-08-25', 40), run('2026-09-01', 30)],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
  ...
  it('derives pace from the week total, and leaves a zero week without one', () => {
    ...
    const table = buildKmTable([run('2026-08-25', 20, 6000)], { limit: 2, currentWeekStart: '2026-08-31' });
  ...
  it('drops a zero-duration row instead of dividing by it', () => {
    ...
    const table = buildKmTable([run('2026-09-01', 12, 0)], { limit: 1, currentWeekStart: '2026-08-31' });
    expect(table[0]).toMatchObject({ km: 0, runs: 0, paceSecPerKm: null });
  });

  it('marks exactly one column current', () => {
    const table = buildKmTable([], { limit: 4, currentWeekStart: '2026-08-31' });
    expect(table.filter((r) => r.isCurrent).map((r) => r.weekStart)).toEqual(['2026-08-31']);
  });

  it('counts only runs, not rides and not zero-distance rows', () => {
    const table = buildKmTable(
      [
        { ...run('2026-09-01', 40), activity_type: 'cycling' },
        { ...run('2026-09-01', 0), distance: 0 },
        run('2026-09-01', 6),
      ],
      { limit: 1, currentWeekStart: '2026-08-31' },
    );
  ...
  it('returns a full window of zeroes for an athlete with no runs', () => {
    ...
    const table = buildKmTable([], { limit: 10, currentWeekStart: '2026-08-31' });
...
describe('pickWeek', () => {
  ...
  it('reads the requested week off the table', () => {
    const table = buildKmTable(
      [run('2026-08-25', 20), run('2026-09-01', 10), run('2026-09-03', 15)],
      { limit: 2, currentWeekStart: '2026-08-31' },
    );
    expect(pickWeek(table, '2026-08-31')).toEqual({ km: 25, runs: 2 });
  });
  ...
  it('returns an explicit zero week when the week is missing', () => {
    ...
    expect(pickWeek([], '2026-08-31')).toEqual({ km: 0, runs: 0 });
  });
});
...
describe('computeLikeForLikeTrend', () => {
  ...
  // 2026-08-31 is a Monday, so 2026-09-02 is the Wednesday of that activity week
  // (daysElapsed = 3, Mon→1) and the comparable slice of the previous week is
  // 2026-08-24 (Mon) through 2026-08-27 exclusive.
  ...
  it('treats Monday as day 1 of the week, not day 2', () => {
    // `daysElapsed` has to count from the ACTIVITY week's own first day. It used to
    // be `getDay() + 1`, which is Sunday→1 — correct only while activity weeks
    // started on Sunday. Left alone after the 2026-09-09 re-split it would slice
    // last week one day too wide, and on a Monday that doubles the denominator:
    // here it would compare 10 km against 8 + 20 = 28 and report −64% to an
    // athlete who is actually 25% ahead.
    const mon = new Date('2026-08-31T09:00:00'); // Monday, day 1 of the 08-31 week
    const trend = computeLikeForLikeTrend(
      [run('2026-08-31T06:00:00', 10), run('2026-08-24T06:00:00', 8), run('2026-08-25T06:00:00', 20)],
      mon,
    );
  ...
  it('keeps a late Sunday-evening run in its own week', () => {
    // The whole reason `activityWeekStart` is used rather than local date
    // getters: read as an instant, a 21:30 run shifts +3h and jumps into the next
    // week, moving kilometres between the two sides of this ratio. On a Mon–Sun
    // activity week the run that would move is the one late on the closing
    // Sunday — 2026-08-30 closes the 2026-08-24 week.
    const trend = computeLikeForLikeTrend(
      [run('2026-08-31T06:00:00', 10), run('2026-08-24T06:00:00', 10), run('2026-08-30T21:30:00', 8)],
      wed,
    );
    // The Sunday run is last week's, and it is past the Wednesday cutoff, so
