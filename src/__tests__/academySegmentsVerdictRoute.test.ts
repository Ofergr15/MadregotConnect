...
describe('GET /api/academy/segments?verdict=1', () => {
  it('grades the run the caller named, not the closest to planned distance', async () => {
    ...
    expect(body.efforts).toMatchObject({ verdict: 'confirmed', neededTotal: 6, foundTotal: 6 });
  });

  it('gives the athlete their own rep paces', async () => {
    const body = await (await call(`athleteId=${ATHLETE}&date=${DATE}&verdict=1&activityId=act-session`)).json();
    expect(body.efforts.requirements[0].paces).toEqual([202, 202, 202, 202, 202, 202]);
  });

  // A teammate gets the plan, never the score. The verdict carries an accuracy
  // PERCENTAGE — a number on a named person, comparable at a glance — so unlike a
  // band on a chart it is not club training content.
  //
  // Answered as `verdict: null` rather than 403 on purpose: the activity detail
  // asks for the bands and the verdict together, and refusing the request would
  // take the chart overlay down with the score. `null` is a state this caller
  // already renders, because it is what a day with no plan returns.
  it('gives a teammate the bands but no verdict', async () => {
    ...
    // And nothing about the run itself came back — not the reps, not the efforts.
    expect(body.efforts).toBeUndefined();
  });

  // Enforced by omission, not by doing the work and dropping it: a score nobody
  // may read should not cost an activity read and a lap match either.
  it("does not touch the run when the verdict is not the caller's to read", async () => {
    mayActFor.mockReturnValue(false);
    await call(`athleteId=${ATHLETE}&date=${DATE}&bands=1&verdict=1&activityId=act-session`);
