...
describe('normalizeStoredLaps', () => {
  ...
  /**
   * Zero is not a heart rate. A lap the strap dropped comes back 0 from both
   * providers, and the run detail's per-split HR chart drew it as a beat count of
   * zero — a cliff to the floor of the axis in the middle of a run. Elevation
   * deliberately does NOT go through the same guard: a flat kilometre really is 0.
   */
  it('treats a heart rate of 0 as no reading, on either side of the seam', () => {
    expect(normalizeStoredLaps([{ distance: 1000, duration: 300, averageHR: 0, maxHR: 0 }])[0])
      .toMatchObject({ averageHR: null, maxHR: null });
    expect(narrowLaps([{ distance: 1000, duration: 300, averageHR: 0, maxHR: 0 }])[0])
      .toMatchObject({ averageHR: null, maxHR: null });
    expect(normalizeStoredLaps([{ distance: 1000, duration: 300, elevationGain: 0 }])[0]
      .elevationGain).toBe(0);
  });
